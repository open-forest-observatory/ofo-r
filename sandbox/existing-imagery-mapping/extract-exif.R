...
# Which folder contans folders of image sets that we want to extract EXIF from (under `datadir`)?
focal_folder = file.path("1_manually-cleaned", "2022-early-regen")

# Which folder do we want to save the extracted data to (under `datadir`)?
out_folder = file.path("extracted-exif", "2022-early-regen")
...
extract_singleset_exif = function(datadir, image_folder, out_folder, sets_to_skip) {
  ...
  out_filepath = file.path(datadir, out_folder, paste0(basename(image_folder), ".csv"))
...
extract_dir_exif = function(datadir, focal_folder, out_folder, sets_to_skip) {

  # Get the subfolders, which are the folders we want to get image EXIF from
  image_folders = list.dirs(file.path(datadir, focal_folder), full.names = TRUE, recursive = FALSE)

  future::plan(future::multisession)
  furrr::future_map(image_folders, extract_singleset_exif, datadir = datadir, out_folder = out_folder, sets_to_skip = sets_to_skip)
  future::plan(future::sequential)

}

extract_dir_exif(datadir, focal_folder, out_folder, sets_to_skip)
...
files = list.files(file.path(datadir, out_folder), pattern = ".csv$", full.names = TRUE, recursive = TRUE)
files
...
readr::write_csv(d, file.path(datadir, "temp", "combined-exif-inspect.csv"))

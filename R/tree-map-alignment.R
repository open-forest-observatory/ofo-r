...
simulate_tree_maps = function(trees_per_ha = 250, trees_per_clust = 5, cluster_radius = 25,
  ...
  pred = spatstat.random::rMatClust(
    kappa = cluster_dens, scale = cluster_radius, mu = trees_per_clust,
    win = spatstat.geom::as.owin(c(
      -pred_extent / 2,
      pred_extent / 2,
      -pred_extent / 2,
      pred_extent / 2
...
find_best_shift_grid = function(pred, obs,
  ...
  if (!obs_inside_pred[1, 1]) {
    stop(
      "The observed tree map (including its base shift of ", base_shift_x, ",", base_shift_y,
  ...
  if (!search_inside_pred[1, 1]) {
    ...
    warning(
      "The specified search window of ", search_window, " is too large. Accounting for the base shift of (",
...
find_best_shift = function(pred, obs,
  ...
  if (method == "grid") {
    ...
    result_coarse = find_best_shift_grid(
      pred = pred, obs = obs,
    ...
    result_fine = find_best_shift_grid(
      pred = pred, obs = obs,
      obs_bounds = obs_bounds,
      objective_fn = objective_fn,
      search_window = 3,
    ...
    result = data.frame(
      shift_x = result_fine$best_shift$shift_x,
...
get_closest_pairs_after_transformation = function(xy_mat1, xy_mat2, R_mat, t_vect, r_thres) {
  ...
  from_idx2_to_min_dist1 = transformed_loc_p_dist[
    cbind(seq_len(nrow(transformed_loc_p_dist)), from_idx2_to_closest_idx1)
  ]

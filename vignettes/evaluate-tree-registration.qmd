---
title: "Evaluating tree registration"
...
There are various approaches to tree registration. The goal of this notebook is to conduct a direct comparison between them. The user can control the types of errors present in the initial registration and define a function that evaluates the quality of the predicted registration. Then, this code runs a set of experiments where a simulated tree alignment problem is generated, each algorithm is run on it, and the results are scored.
...
## Define the registration algorithms to compare

```{r}
# The actual functions that take in a predicted and observed tree map and output a transform between them
REGISTRATION_METHODS = c(find_best_shift, find_best_shift)
# Additional arguments that are provided to these functions to control their behavior
PER_METHOD_REGISTRATION_ARGUMENTS = list(list(method="optim"), list(method="grid"))
# The name of these methods for visualization
REGISTRATION_METHOD_NAMES = c("find_best_shift_optim", "find_best_shift_grid")
```
...
Conduct all the registration trials with the set of starting configurations and multiple algorithms.

```{r}
all_registration_results = test_tree_registration(
 map_params = MAP_PARAMS,
 registration_methods = REGISTRATION_METHODS,
 registration_method_names = REGISTRATION_METHOD_NAMES,
 per_method_registration_arguments = PER_METHOD_REGISTRATION_ARGUMENTS,
...
These are used only to visualize the results. They take the true and predicted registration values and use them to compute a scalar metric representing the quality.
...
# for the other metric, the same is here, with 0 representing suffient registration and 1
...
visualize_metric_vs_1_parameter(
  results_df = all_registration_results$per_method_results$find_best_shift_optim,
  parameter_df = all_registration_results$all_map_param_configurations,
...
# Grid search
visualize_metric_vs_1_parameter(
  results_df = all_registration_results$per_method_results$find_best_shift_grid,
  parameter_df = all_registration_results$all_map_param_configurations,
  attributes_to_visualize = c("shift_x", "horiz_jitter"),
  eval_function = eval_res_threshold,
  smoothing_bandwidth = c(10,2),
...
Show the registration error versus parameter values. Lower is better.
